...
void plyrTrn(char a[], int b[], int c[], int d[], int &canvas,int start, int end);
...
int main(int argc, char** argv) {
    ...
    int  iHand[16], //Player 1 unique number per card[0-7],  Player 2 unique number per card [8-15]
         ...
         canvas[16]={0}, //Player 1 cin canvas change choice[0-7], player 2 cin canvas change choice [8-15]
...
//         iTemp, tempCol; //variables to assign beginning starting hands
    ...
    do{
        ...
        if (turn%2==1)
        plyrTrn(display,palet,canvas,cPalet,canvas1,0,7);
        else if (turn%2==0)
        plyrTrn(display,palet,canvas,cPalet,canvas1,8,15);
        if (canvas1 == RED)
            red(palet,cPalet);
