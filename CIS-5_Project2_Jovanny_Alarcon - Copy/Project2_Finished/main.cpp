...
bool linSrch(const char, const int, string [],const char[], const int[], const int, const int, const char, const int = 0); //Linear Search used in function plyrTurn to check if played card is in players hand.
    ...
    for(int i = start; i < end; i++){
        ...
        if(b[i]  ==  0 && c[i]  ==  0 && count < 2){
            ...
            if (porc  ==  'P'){
                ...
                }while(!linSrch( a[i], b[i], h, cHand, iHand, start, end, c[i])); //Linear Search used in function plyrTurn to check if played card is in players hand.
                count++;
            }
            ...
            //When successful the counter will increase to indicate they have played a card. After playing 2 the program prompts the next player's turn
                ...
                do{
                    ...
                    canvas = c[i];
                    cin >> dummy;
                }while(!linSrch( a[i], b[i], h, cHand, iHand, start, end, temp, dummy)); //Linear Search used in function plyrTurn to check if played card is in players hand.
                count++;
            }
...
void red(const int a[], const int b[]){
    //Finds player 1's highest value card
    int hghVal1, highC1;
    ...
    //Finds player 2's highest value card
    int hghVal2, highC2;
...
void orange(const int a[],const int b[]){
    //Finds the highest set of cards of one number and from those finds the highest value card for Player 1
    ...
    //Finds the highest set of cards of one number and from those finds the highest value card for Player 2
    ...
    for (int i=1; i<=7;i++){
        ...
        winChck(mxCnt1,mxCnt2,hghVal1,hghVal2,highC1,highC2); //Function that compares both players Palette
}
...
void yellow(const int a[],const int b[]){
    ...
    int mxCnt1=0,  //Max count of cards of one color
        hghVal1=0, //Highest value from Max Count
    ...
    int mxCnt2=0,  //Max count of cards of one color
        hghVal2=0, //Highest value from Max Count
...
//Function pulls array from main and creates a new array from player inputted cards for both players
//Uses Selection Sort to sort the cards based off value and then color
//This function sets up indigo function to check for the highest running set of cards
void srtCrds(const int a[],const int b[],int size){
    //Player 1 inputted Vector
    ...
    //Sorts Player 1 inputted Vector
...
void indigo(int a[],int b[],int c[],int d[],int count1,int count2){
    ...
    //Finds player 1's highest set of running cards as well as the highest value/color card i.e. tie breaker
    ...
    //Finds player 2's highest set of running cards as well as the highest value/color card i.e. tie breaker
...
//Function called by the canvas rules (except Red) and determines who wins
...
//If the player has no valid cards to win the Round they are given to option to resign
...
//This function lets the next player have their turn and displays the current turn round
...
//If the player gives Up then the game is concluded, an output file is created displaying the winner
//That output file is read in by an input files and displays in the string victor
//The games then exits with the exit function
...
//Return function that checks if the player inputted card is part of their hand set
bool linSrch(const char color, const int value, string hand[], const char cHand[],const int iHand[], const int start,  const int end, const char canColr, const int dummy){
    int index; //Array element that starts at either 0 for player 1 or 8 for player 2
    int position = -1; //Variable that will change if the card is in one of the elments of the array
    bool isInHnd = false; //Set bool to false unless card is found
    //Checks player inputted Canvas is in their hand, returning false will have them re-input a card
    if (canColr != 0){
        for (index = start; index < end; index++){
            if (canColr == cHand[index]){
                if(dummy == iHand[index]){
                    position = index;
                    hand[position] = "";
                    isInHnd = true;
                }
            }
            if (position == -1 && index == end - 1)
                cout << " This card is not in your hand. Play another card. ";
        }
    }
    //Checks player inputted Palette is in their hand, returning false will have them re-input a card
    else{
        for (index = start; index < end; index++){
    ...
    return isInHnd; //Returns true or false to possibly reiterate inputting the correct values
}
